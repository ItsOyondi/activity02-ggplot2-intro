...
**Response**: ggplot automatically assigned unique colors to each
species and also provided a key to identify the species. This is helpful
in differentiating the species on the visualization.
...
**Response**:With boxplot, it is easier to determine the approximate
(flipper length in mm) minimum and maximum values unlike in the
jitterplot whereby the values are scattered across the distribution. On
the other hand, at a glance, it is easier to determine the position of
distributions of each species while using jitterplots faster than in the
boxplot.
...
**Response**: .md is easier to read and more professional than the .Rmd
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species, color = species, position = "jitter")) +
...
**Response**: I prefer jitterplot over boxplot than the boxplot over
jitterplot. The distribution of values is not hidden and the
